    ...
    transport.execute(req, function(code, resp, resp_headers, err)
        ...
        if code == 200 or code == 20000 then
            ...
            if ok and parsed and parsed.data and parsed.data.devices then
                ...
                -- Look for the K15 camera device by IP match first, then by product_subtype
                ...
                for i, device in ipairs(devices) do
                    ...
                    elseif not ip and device.model and string.find(string.lower(device.product_subtype), string.lower(GlobalObject.ProductSubType)) then
                        target_device = device
                        print("Found K15 camera device (no IP filter) at index " .. i .. ": " .. (device.model or "unknown model"))
                        break
                    end
                end
