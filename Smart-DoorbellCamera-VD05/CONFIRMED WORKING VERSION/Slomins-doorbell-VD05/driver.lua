...
local WAKE_DURATION = 60   -- Camera stays awake for 60 seconds (battery cameras need longer)
    ...
    if not IsAwake() then
        ...
        transport.execute(req, function(code, resp, resp_headers, err)
            if code == 200 or code == 20000 then
                ...
                -- Wait 5 seconds for camera to fully wake and start streaming
...
function OnDriverLateInit()
    ...
    if ip and ip ~= "" then
        ...
        C4:UpdateProperty("Sub Stream URL", string.format("rtsp://%s:%s/stream1", ip, rtsp_port))
...
function OnPropertyChanged(strProperty)
    ...
    if strProperty == "IP Address" and value and value ~= "" then
        ...
        if auth_required and username ~= "" and password ~= "" then
            rtsp_main = string.format("rtsp://%s:%s@%s:%s/stream0", username, password, value, rtsp_port)
            rtsp_sub = string.format("rtsp://%s:%s@%s:%s/stream1", username, password, value, rtsp_port)
        else
            rtsp_main = string.format("rtsp://%s:%s/stream0", value, rtsp_port)
...
function UIRequest(strCommand, tParams)
    ...
    -- Check if camera needs to wake for video streaming
    if strCommand == "GET_RTSP_H264_QUERY_STRING" then
        if not IsAwake() then
            ...
            AutoInitialize(function(success)
                if success then
                    WakeIfNeeded(function(wake_success)
                        if wake_success then
                            C4:UpdateProperty("Status", "Camera awake - ready for video")
                        end
                    end)
                end
            end)
            
            -- Return empty path to force Control4 to retry after wake
            -- if strCommand == "GET_RTSP_H264_QUERY_STRING" then
            --     print("Returning empty RTSP path - Control4 will retry in 3-5 seconds")
            --     return "<rtsp_h264_query_string></rtsp_h264_query_string>"
            -- end
        else
            print("Camera is awake - RTSP server ready")
        end
    end
...
function ReceivedFromProxy(idBinding, strCommand, tParams)
    ...
    print("================================================================")
        ...
        -- Auto-initialize for other commands
    AutoInitialize(function(success) end)

    -- Check if camera needs to wake for video streaming
    if strCommand == "GET_RTSP_H264_QUERY_STRING" or strCommand == "GET_MJPEG_QUERY_STRING" then
        if not IsAwake() then
            C4:UpdateProperty("Status", "Waking camera (3-5 sec)")
            ...
            AutoInitialize(function(success)
                if success then
                    WakeIfNeeded(function(wake_success)
                        if wake_success then
                            print("Camera wake complete - RTSP server ready")
                            C4:UpdateProperty("Status", "Camera awake - ready for video")
                        end
                    end)
                end
...
function GET_STREAM_URLS(idBinding, tParams)
    ...
    -- Build RTSP URLs for VD05 Video Doorbell
...
function GET_RTSP_H264_QUERY_STRING(idBinding, tParams)
    ...
    -- Camera Proxy will build: rtsp://username:password@ip:port/streamX
...
function GET_MJPEG_QUERY_STRING(idBinding, tParams)
    ...
    -- VD05: MJPEG is not supported by this camera (low-power, battery-operated doorbell)
    -- Only RTSP streaming is available
    print("MJPEG streaming not supported on VD05 video doorbell")
