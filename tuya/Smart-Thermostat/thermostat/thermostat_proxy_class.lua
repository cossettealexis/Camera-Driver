...
if (TEMPLATE_VERSION ~= nil) then
	TEMPLATE_VERSION.thermostat_proxy_class = "2015.03.02"
end
...
function ThermostatProxy:Initialize()
	self._HeatSetpoint = 22
	self._CoolSetpoint = 25
	self._SingleSetpoint = true
	self._Temperature = 23
	self._OutdoorTemperature = 32
	self._Scale = "C"
	self._HVACState = "Auto"
	self._FanMode = ""
	self._FanState = ""
	self._HoldMode = ""
	...
	-- Lists
	self._HVACModes = "Off,Heat,Cool,Auto"
	self._CanHeat = true
	self._CanCool = true
	self._CanAuto = true
...
function ThermostatProxy:prx_SET_SETPOINT_HEAT(tParams)
	...
	if (celsius ~= nil and fahrenheit ~= nil) then
		...
		print('Set Heat Temp ' ..tostring(fahrenheit))
		gTStatProxy:dev_HeatSetpoint(celsius, "C")
	end
end
...
function ThermostatProxy:prx_SET_SETPOINT_COOL(tParams)
	...
	if (celsius ~= nil and fahrenheit ~= nil) then
		...
		print('Set Cool Temp ' ..tostring(fahrenheit))
		gTStatProxy:dev_CoolSetpoint(celsius, "C")
	end
end
...
function ThermostatProxy:prx_SET_SETPOINT_SINGLE(tParams)
	...
	if (celsius ~= nil and fahrenheit ~= nil) then
		SET_SETPOINT_SINGLE(celsius, fahrenheit)
	end
end
...
function ThermostatProxy:prx_SET_OUTDOOR_TEMPERATURE(tParams)
	...
	if (celsius ~= nil and fahrenheit ~= nil) then
		SET_OUTDOOR_TEMPERATURE(celsius, fahrenheit)
	end
end

-- SET_PRESETS is not supported with this version of the template
function ThermostatProxy:prx_SET_PRESETS(tParams)
end

-- SET_PRESET is not supported with this version of the template
function ThermostatProxy:prx_SET_PRESET(tParams)
end

-- SET_EVENTS is not supported with this version of the template
function ThermostatProxy:prx_SET_EVENTS(tParams)
end

-- SET_EVENT is not supported with this version of the template
function ThermostatProxy:prx_SET_EVENT(tParams)
end

-- not to be used with Preset Scheduling
