...
function UpdateAuthToken(token)
    ...
    -- Enable MQTT only once after first auth
    if not MQTT_AUTO_ENABLED and Properties["Enable MQTT"] ~= "True" then
        print("[MQTT] Auto enabling MQTT (first time after auth)")
...
function PTZ_COMMAND(idBinding, strCommand, tParams)
    ...
    -- Map Control4 commands to numeric direction + state
    local direction_map = {
        PAN_LEFT   = 2,   -- Left
        PAN_RIGHT  = 3,   -- Right
        TILT_UP    = 0,   -- Up
        TILT_DOWN  = 1,   -- Down
        -- ZOOM_IN and ZOOM_OUT are NOT supported by ac_ptz
    }
    ...
    if not direction then
        ...
        if strCommand == "ZOOM_IN" or strCommand == "ZOOM_OUT" then
            print("Note: Zoom is not supported by this camera's ac_ptz action.")
        end
        return
    end

    -- For normal PTZ buttons in Control4, we do continuous move (state = 1)
    local state = 1

    print("PTZ → Direction:", direction, "(0=Up,1=Down,2=Left,3=Right)  State: 1 (Move)")
    ...
    local body = {
        vid = vid,
        action_id = "ac_ptz",
        input_params = json.encode(input_params),   -- MUST be JSON string
        check_t = 0,
        is_async = 0
    }
    ...
    transport.execute({
        ...
        if code == 200 or code == 20000 then
            print("PTZ command successful: " .. strCommand)
            C4:UpdateProperty("Status", "PTZ " .. strCommand)
        else
            print("PTZ command failed - Code:", code)
...
function PTZ_STOP(idBinding, tParams)
    ...
    local input_params = {
        direction = 0,   -- direction doesn't really matter for stop
        state = 0        -- 0 = stop
    }
