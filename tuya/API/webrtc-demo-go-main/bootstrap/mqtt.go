...
func GetMotoIDAndAuth(deviceID string) (motoID, auth, iceServers string, err error) { 
	url := fmt.Sprintf("https://%s/v1.0/users/%s/devices/%s/webrtc-configs", config.App.OpenAPIURL, config.App.UID, deviceID)
