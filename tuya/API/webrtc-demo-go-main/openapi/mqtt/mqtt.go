...
func Start(deviceID string) (err error) {
	motoID, auth, iceServers, err = bootstrap.GetMotoIDAndAuth(deviceID)
	if err != nil {
	...
	publishTopic = strings.Replace(publishTopic, "{device_id}", deviceID, 1)
...
func FetchWebRTCConfigs(deviceID string) (err error) {
	_, _, iceServers, err = bootstrap.GetMotoIDAndAuth(deviceID)
	if err != nil {
