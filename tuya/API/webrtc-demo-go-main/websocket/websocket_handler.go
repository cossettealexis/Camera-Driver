...
func dispatch(msg *types.WsMessage) {
	...
	case "mqtt":
		...
		if msg.Type == "webRTCConfigs" { 
			...
			log.Printf("Received deviceID from frontend: %s", deviceID)

			// Start mqtt client with deviceID
			if err := openmqtt.Start(deviceID); err != nil {
				log.Printf("start mqtt fail: %s", err.Error())
				return
			}

			if err := openmqtt.FetchWebRTCConfigs(msg.DeviceID); err != nil {
