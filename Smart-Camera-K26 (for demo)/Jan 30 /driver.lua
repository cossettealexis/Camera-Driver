...
-- Snapshot cache for Navigator list view
-- Initialize with default path so list view NEVER wakes camera (uses stale snapshot)
local cached_snapshot = ""  -- Default stale snapshot path
local cached_snapshot_time = 0
local SNAPSHOT_CACHE_DURATION = 300  -- Cache for 5 minutes (300 seconds)
...
function WakeCamera(retry)
    ...
    local function try_wake()
        ...
        if attempt < retry then
            ...
            print("Wake retry " .. retry .. " times done (WAKE DISABLED)")
        end
    end
...
-- Stop RTSP wake polling when streaming session ends
function StopRTSPWakePolling()
    if rtsp_wake_timer then
        print("[RTSP] Stopping wake polling - streaming session ended")
        rtsp_wake_timer:Cancel()
        rtsp_wake_timer = nil
    end
    
    if rtsp_timeout_timer then
        print("[RTSP] Canceling timeout timer")
        rtsp_timeout_timer:Cancel()
        rtsp_timeout_timer = nil
    end
    
    C4:UpdateProperty("Status", "Streaming stopped, camera will sleep")
end
...
function AUTO_START_AUTH_FLOW()
    -- 🚫 Never run again once completed
    if AUTO_FLOW_COMPLETED then
        print("[AUTO] Flow already completed, skipping")
        return
    end

    if _initializing then
        print("[AUTO] Flow already running, skipping")
        return
    end
    ...
    if not Properties["Public Key"] or Properties["Public Key"] == "" then
        print("[AUTO] Step 1: InitializeCamera")
        InitializeCamera()
        _initializing = false
        return
    end
    ...
    if not Properties["Auth Token"] or Properties["Auth Token"] == "" then
        ...
        if account and account ~= "" then
            print("[AUTO] Step 2: LoginOrRegister")
    ...
    if not mqtt_enabled then
        print("[AUTO] Step 3: Enabling MQTT")
    ...
    -- ✅ Done forever
    AUTO_FLOW_COMPLETED = true
    _initializing = false
    print("[AUTO] ✅ Auth + MQTT flow complete (locked)")
end
...
function OnDriverLateInit()
    ...
    if not AUTO_FLOW_STARTED then
        ...
        C4:SetTimer(2000, function()
            print("[AUTO] Starting auto-auth + MQTT flow")
            AUTO_START_AUTH_FLOW()

            -- stop blocking user MQTT toggle after 15s
            C4:SetTimer(15000, function()
                _initializing = false
                print("[AUTO] Initialization window closed")
            end)
        end)
    end
...
function SendUpdateCameraProp(extractedData)
    ...
    local camera_props = {
        ...
        device_name = cameraData.device_name or Properties["Device Name"] or "Slomins K26-SL Outdoor Camera",
        ...
        if code == 200 or code == 20000 then
            ...
            if ok and parsed and parsed.data and parsed.data.devices then
                ...
                for i, device in ipairs(devices) do
                    ...
                    if (device.model and string.find(string.lower(device.model), "k26")) or
                        (device.product_subtype and device.product_subtype == "solar_box_cam") then
...
function GET_CAMERA_PROPERTIES()
    ...
    local camera_props = {
        ...
        device_name = Properties["Device Name"] or "Slomins K26-SL Outdoor Camera",
...
function GET_RTSP_H264_QUERY_STRING(idBinding, tParams)
    ...
    -- Wake camera immediately for RTSP stream
    -- print("[RTSP] Waking camera for streaming session...")
    -- WakeCamera(1)

    -- Cancel any existing wake timer
    -- if rtsp_wake_timer then
    --     print("[RTSP] Canceling previous wake polling timer")
    --     rtsp_wake_timer:Cancel()
    --     rtsp_wake_timer = nil
    -- end
    ...
    -- if rtsp_timeout_timer then
    --     print("[RTSP] Canceling previous timeout timer")
    --     rtsp_timeout_timer:Cancel()
    --     rtsp_timeout_timer = nil
    -- end

    -- Start periodic wake polling to keep camera awake during streaming
    -- print("[RTSP] Starting wake polling every 2 minutes...")
    -- rtsp_wake_timer = C4:SetTimer(RTSP_WAKE_INTERVAL, function(timer)
    --     print("[RTSP] Keep-alive wake (2 min interval)")
    --     WakeCamera(1)
    -- end, true)  -- true = repeating timer
    
    -- Set timeout to auto-stop after 10 minutes
    -- print("[RTSP] Setting 10 minute timeout for auto-stop")
    -- rtsp_timeout_timer = C4:SetTimer(RTSP_TIMEOUT, function(timer)
    --     print("[RTSP] TIMEOUT: Auto-stopping wake polling after 10 minutes")
    --     StopRTSPWakePolling()
    -- end)
